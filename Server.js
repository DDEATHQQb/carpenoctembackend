...
io.on("connection", socket => {
  ...
  socket.on("enterGroup", data => {
    ...
    const loadMsg =
      "SELECT message FROM ChatLog WHERE messageId in (SELECT messageId FROM Chat WHERE ChatuserId = ? and ChatgroupId = ?);";
  ...
  socket.on("joinGroup", data => {
    const sql =
      "INSERT INTO JoinGroup(JGuserId,JGgroupID,isExit,latestTimeRead) VALUES(?,?,0,0);";
